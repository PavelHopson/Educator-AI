...
export const ProductionTrack: React.FC<{ language: Language; onExit: () => void }> = ({ language, onExit }) => {
  ...
  return <div className="max-w-5xl mx-auto space-y-5">
    <button type="button" onClick={onExit} className="rounded-lg px-2 py-1 text-sm text-quest-200 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-quest-300">← {language === 'ru' ? 'К курсам' : 'Back to courses'}</button>
    ...
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5" aria-label="Production track stages">{PRODUCTION_TRACK_MODULES.map((item, index) => <button type="button" key={item.id} onClick={() => { setActive(index); setEvidence({}); setMessage(''); }} aria-current={active === index ? 'step' : undefined} className={`min-h-24 rounded-xl border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-quest-300 ${active === index ? 'border-quest-400 bg-quest-500/15' : 'border-quest-700/30 bg-quest-950/30'}`}><span className="text-xs text-quest-300">0{index + 1}</span><strong className="mt-2 block text-sm text-white">{item.title}</strong><span className={progress.completed.includes(item.id) ? 'mt-2 block text-xs text-emerald-300' : 'mt-2 block text-xs text-slate-400'}>{progress.completed.includes(item.id) ? 'Completed' : 'Open'}</span></button>)}</div>
    <section className="quest-card rounded-2xl p-6 md:p-8" aria-live="polite">
      ...
      <div className="mt-5 grid gap-4">{module.evidence.map((key) => <label key={key} className="block text-sm text-slate-200">{key}<textarea value={evidence[key] || ''} disabled={complete} onChange={(event) => { setEvidence((value) => ({ ...value, [key]: event.target.value })); setMessage(''); }} rows={2} className="mt-2 w-full resize-y rounded-xl border border-quest-700/30 bg-quest-950/50 px-4 py-3 text-white disabled:opacity-50" placeholder={language === 'ru' ? 'Запишите проверяемый результат…' : 'Record verifiable evidence…'} /></label>)}</div>
